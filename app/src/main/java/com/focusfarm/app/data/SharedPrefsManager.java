...
public final class SharedPrefsManager {
    ...
    public static long getTemporaryUnblockUntil(Context context, String packageName) {
        return getPrefs(context).getLong(KEY_UNBLOCK_PREFIX + packageName, 0L);
    }

    public static void clearTemporaryUnblock(Context context, String packageName) {
        getPrefs(context).edit()
                .remove(KEY_UNBLOCK_PREFIX + packageName)
                .apply();
    }
