...
import com.focusfarm.app.data.QuizBank;
import com.focusfarm.app.data.QuizQuestion;
...
public class AntiBrainRotService extends AccessibilityService {
  ...
  private TextView quizParagraphView;
  private TextView quizQuestionView;
  private final MaterialButton[] quizAnswerButtons = new MaterialButton[3];
  ...
  private Runnable unblockExpiryRunnable;
  private String scheduledUnblockPackage = "";
  private long unblockScheduleGeneration;
  ...
  public void onAccessibilityEvent(AccessibilityEvent event) {
    ...
    if (SharedPrefsManager.isTemporarilyUnblocked(this, currentApp)) {
      long unblockUntil = SharedPrefsManager.getTemporaryUnblockUntil(this, currentApp);
      long remainingMs = unblockUntil - System.currentTimeMillis();
      if (remainingMs <= 0L) {
        onTemporaryUnblockExpired(currentApp);
      } else {
        scheduleUnblockExpiry(currentApp, remainingMs);
      }
      return;
    }
  ...
  private void showInterventionOverlay(String packageName) {
    ...
    quizParagraphView = overlayView.findViewById(R.id.tv_quiz_paragraph);
    quizQuestionView = overlayView.findViewById(R.id.tv_quiz_question);
    quizAnswerButtons[0] = overlayView.findViewById(R.id.btn_quiz_answer_1);
    quizAnswerButtons[1] = overlayView.findViewById(R.id.btn_quiz_answer_2);
    quizAnswerButtons[2] = overlayView.findViewById(R.id.btn_quiz_answer_3);
  ...
  private void applyContinueUseSeconds(int seconds) {
    ...
    grantTemporaryUnblock(durationMs, R.string.toast_use_app_seconds, seconds);
  }

  private void grantTemporaryUnblock(long durationMs, int toastResId, int toastArg) {
    SharedPrefsManager.setTemporaryUnblockMs(this, currentPackageName, durationMs);
    scheduleUnblockExpiry(currentPackageName, durationMs);
    Toast.makeText(this, getString(toastResId, toastArg), Toast.LENGTH_SHORT).show();
    hideOverlay();
  }

  private void scheduleUnblockExpiry(String packageName, long durationMs) {
    cancelUnblockExpirySchedule();
    if (packageName == null || packageName.isEmpty() || durationMs <= 0L) {
      return;
    }
    final long generation = ++unblockScheduleGeneration;
    final String pkg = packageName;
    scheduledUnblockPackage = pkg;
    unblockExpiryRunnable = () -> {
      if (generation != unblockScheduleGeneration) {
        return;
      }
      onTemporaryUnblockExpired(pkg);
    };
    mainHandler.postDelayed(unblockExpiryRunnable, durationMs);
  }

  private void cancelUnblockExpirySchedule() {
    if (unblockExpiryRunnable != null) {
      mainHandler.removeCallbacks(unblockExpiryRunnable);
    }
    unblockExpiryRunnable = null;
    scheduledUnblockPackage = "";
  }

  /** Called when the temporary unblock window ends — re-block if the user is still in the app. */
  private void onTemporaryUnblockExpired(String packageName) {
    if (packageName == null || packageName.isEmpty()) {
      return;
    }

    long unblockUntil = SharedPrefsManager.getTemporaryUnblockUntil(this, packageName);
    long remainingMs = unblockUntil - System.currentTimeMillis();
    if (remainingMs > 0L) {
      scheduleUnblockExpiry(packageName, remainingMs);
      return;
    }

    cancelUnblockExpirySchedule();
    SharedPrefsManager.clearTemporaryUnblock(this, packageName);
    Set<String> blockedApps = SharedPrefsManager.getBlockedApps(this);
    if (blockedApps.isEmpty() || !blockedApps.contains(packageName)) {
      return;
    }
    ...
    Toast.makeText(this, R.string.toast_unblock_expired, Toast.LENGTH_LONG).show();
  ...
  private void startQuiz() {
    ...
    currentQuiz = QuizBank.getRandomQuestion(random);
    bindQuizUi(currentQuiz);
    layoutQuiz.setVisibility(View.VISIBLE);
  }

  private void bindQuizUi(@NonNull QuizQuestion quiz) {
    if (quizParagraphView != null) {
      quizParagraphView.setText(quiz.paragraphRes);
    }
    if (quizQuestionView != null) {
      quizQuestionView.setText(quiz.questionRes);
    }
    for (int i = 0; i < quizAnswerButtons.length; i++) {
      MaterialButton button = quizAnswerButtons[i];
      if (button == null) {
        continue;
      }
      button.setText(quiz.answerRes[i]);
      button.setAlpha(1f);
      int selectedIndex = i;
      button.setOnClickListener(v -> onQuizAnswer(selectedIndex));
    }
  }

  private void onQuizAnswer(int selectedIndex) {
    if (currentQuiz == null) {
      return;
    }
    if (!currentQuiz.isCorrect(selectedIndex)) {
  ...
  /** After completing the random challenge — short unlock (seconds, for testing). */
  private void showTaskPassedDialog() {
    ...
    showDurationPickerDialog(
            ...
            seconds -> {
              SharedPrefsManager.recordResistChoice(this);
              grantTemporaryUnblock(seconds * 1000L, R.string.toast_use_app_seconds, seconds);
            });
  }
  ...
  private void clearInterventionReferences() {
    ...
    layoutQuiz = null;
    quizParagraphView = null;
    quizQuestionView = null;
    for (int i = 0; i < quizAnswerButtons.length; i++) {
      quizAnswerButtons[i] = null;
    }
    currentQuiz = null;
